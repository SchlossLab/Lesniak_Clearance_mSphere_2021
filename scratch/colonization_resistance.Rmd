...
for(df in list(pre_post_end_phyla_df, pre_post_end_genus_df, pre_post_end_otu_df)){
  ...
  #    scale_fill_brewer(palette = 'Paired')
    ...
    daily_plot <- all_days_otu_df %>% 
      filter(abx == antibiotic) %>% 
      mutate(taxa = factor(taxa, c('Other', unique(all_days_otu_df$taxa)[unique(all_days_otu_df$taxa)!='Other']))) %>% 
      ggplot(aes(x = mouse_position, y = abundance, fill = taxa)) + 
        geom_bar(stat="identity", position='stack', width = 1, color = "black", size = 0.1) + 
...
#     plot_grid(plot_header, plot_grid(pre_post_end_plot, cdiff_plot, rel_widths = c(2, 1)), 
#       ncol = 1, rel_heights = c(0.1, 2)),
#     width = 10, height = 10)
    ggsave(paste0('../results/figures/daily_plots/', antibiotic, '_', unique(df$dataset), '.jpg'),
      plot_grid(plot_header, plot_grid(daily_plot, cdiff_plot, rel_widths = c(5, 1)), 
        ncol = 1, rel_heights = c(0.1, 2)),
      width = 20, height = 10)
  }
}
...
#### Questions
  - Is there a way to explain/predict the way a community responds to Ampicilin?
  - What are the commonalities between all the infected communities?
...
- Ampicilin (dose 0.5)
    - Ampicilin treatment lead to very different communities so comparing the communites that looked similar on day 0, there looked fairly different at the endpoint
...
Look at the community over time in treatments with differing levels of colonization or clearance
Clear
  - Clindamycin (clinda_10_FALSE_TRUE)
  - 
Different Colonization
  - 
Decreasing Colonization

Maintain colonization with changing community
  - Amp_0.5 - different communities infected and shift, infection maintained > 10^6, recovery allowed the community to shift but lead to 3 different communities which were all infected but at slightly lower levels than no recovery
Maintain colonization with same community

```{r community recovery}

compare_community_colonization <- function(selected_treatment, taxonomic_level, n_taxa){
	treatment_df <- meta_df %>% 
	  filter(treatment %in% c(selected_treatment)) %>% 
	  left_join(
	    sum_otu_by_taxa(taxonomy_df = taxonomy_df, otu_df = rel_abund, 
	        taxa_level = taxonomic_level, top_n = n_taxa),
	  by = 'group') %>% 
	  mutate(dataset = paste0(taxonomic_level, '_level'))
	
	if(taxonomic_level == 'tax_otu_label'){
	...
	community_plot <- treatment_df %>% 
	  mutate(taxa = factor(taxa, c('Other', unique(treatment_df$taxa)[unique(treatment_df$taxa)!='Other']))) %>% 
	  ggplot(aes(x = mouse_position, y = abundance, fill = taxa)) + 
	    geom_bar(stat="identity", position='stack', width = 1, color = "black", size = 0.1) + 
	    facet_grid(treatment ~ day) + 
	    theme_bw() + labs(x = NULL) + 
	...
	cdiff_plot <- treatment_df %>% 
	  group_by(mouse_position, treatment, day, mouse_id) %>% 
	  summarise(CFU = median(CFU) + 1) %>% 
	  ggplot(aes(x = mouse_position, y = CFU)) + 
	    geom_bar(stat="identity", width = 1) + 
	    facet_grid(treatment ~ day) + 
	    scale_y_log10() + 
	    theme_bw() + labs(x = NULL, y = 'C. difficile CFU (log10)') + 
	    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

	plot_header <- ggdraw() +
	 draw_label(paste0('Daily Community overview ', selected_treatment, ' looking at the ', 
	    taxonomic_level), x = 0.05, hjust = 0)

    ggsave(paste0('../results/figures/daily_plots/colonization_dynamics', selected_treatment, '_', taxonomic_level, '.jpg'),
      plot_grid(plot_header, plot_grid(community_plot, cdiff_plot, ncol = 1), 
        ncol = 1, rel_heights = c(0.1, 2)),
      width = 20, height = 10)
  }
#compare_community_colonization(selected_treatment = 'cef_0.3_FALSE_TRUE', taxonomic_level = 'Phylum', n_taxa = 6)
...
for(trt in the_list){
	for(tax in 1:6){
		level <- c('Phylum','Class','Order','Family','Genus', 'tax_otu_label')[tax]
		num <- c(6,8,8,10,11,11)[tax]
		compare_community_colonization(selected_treatment = trt, 
			taxonomic_level = level, n_taxa = num)
	}
