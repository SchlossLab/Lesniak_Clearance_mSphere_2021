...
phyla_rel_ab <- left_join(sum_otu_by_taxa(taxonomy_file = tax_file, otu_df = rel_abund,taxa_level = 'phylum'),
  select(shared_cdiff, group, C.difficile), by = 'group')
meta_phyla <- left_join(hi_lo_no_delay_df, phyla_rel_ab, by = 'group')
taxa <- colnames(select(phyla_rel_ab, -group))
...
# plot by dose, but only looking at one dosage in current analysis
#
#hi_lo_no_delay_df %>% 
#  filter(day >= 0) %>%
...
p <- abx_v_final_alpha %>% 
  ggplot(aes(x = time_point, y = value, group = interaction(cdiff, time_point))) + 
    ...
    labs(x = 'Time Point of Diversity Metric', y = NULL) + 
    expand_limits(y = 0) + 
    theme_bw()
```
...
- Check to make sure all make it to day 10 and how many dont
...
p + geom_point(position = position_jitterdodge(dodge.width = .9, jitter.width = 0.2), 
    ...
    labs(title = 'Alpha diversity of community comparing antibiotic treatment and colonization',
      subtitle = 'Unchallenged (left bar) and C difficile Challenged (right bar)\nColored by CFU at time point')
```
...
p + geom_point(position = position_jitterdodge(dodge.width = .9, jitter.width = 0.2), 
    ...
    labs(title = 'Alpha diversity of community comparing antibiotic treatment and colonization',
      subtitle = 'Unchallenged (left bar) and C difficile Challenged (right bar)\nColored by CFU at end time point')
```
...
abx_v_final_alpha %>% 
  ...
  ggplot(aes(x = time_point, y = value, color = hi_lo, group = interaction(hi_lo, time_point))) + 
    ...
    geom_point(position = position_jitterdodge(dodge.width = .9, jitter.width = 0.2), 
      alpha = 0.5) + 
    expand_limits(y = 0) + 
    theme_bw() + labs(title = 'Comparing Alpha diversity of communities infected with C. difficile\nby their endpoint colonization level', y = NULL, x = 'Time Point of Diversity Metric', color = 'Endpoint \nColonization',
...
abx_v_final_alpha %>% 
  ...
  ggplot(aes(x = abx, y = value, group = interaction(abx, hi_lo, cdiff))) + 
    ...
    labs(title = 'Comparing Alpha diversity of communities infected with \n C. difficile by their endpoint colonization level', 
      x = 'Antibiotic Treatment', y = NULL, color = '',
      subtitle = paste0('Low = Below level of Detection, High = Greater than 1e+0', 
        signif(mean(pull(filter(abx_v_final_alpha, cdiff == T), log_final_CFU)),1)
        )) + 
...
```{r alpha of endpoint by abx and colonization, fig.width = 10, fig.height = 10}
# plot and color by CFU of cdifficile on that day
plot_by_abx_grp <- abx_v_final_alpha %>% 
  ...
  ggplot(aes(x = cdiff, y = value, group = interaction(abx_colonized_grp, cdiff))) + 
    ...
    geom_point(position = position_jitterdodge(dodge.width = .9, jitter.width = 0.1), 
      alpha = 0.5, aes(color = hi_lo)) + 
    expand_limits(y = 0) + theme_bw() + theme(legend.position = 'bottom') + 
    labs(title = 'Alpha diversity of communities\nby antibiotic grouped by colonization level', 
      x = 'C difficile Challenged', y = 'Time Point of Diversity Metric', color = '',
      subtitle = paste0('Low = Below level of Detection\nHigh = Greater than 1e+0', 
        signif(mean(pull(filter(abx_v_final_alpha, cdiff == T), log_final_CFU)),1)
        )) + 
    scale_color_manual(breaks = c('FALSE', 'TRUE', 'High', 'Low'),
      labels = c('No CDI', 'CDI', 'High CFU', 'Low CFU'),
      values = c('gray', 'red', 'blue', 'black'))
...
#  bind_rows(data.frame(cdiff = FALSE, hi_lo = 'High', diversity = 'sobs')) %>% 
  ggplot(aes(x = cdiff, y = value, group = interaction(hi_lo, cdiff))) + 
    ...
    geom_point(position = position_jitterdodge(dodge.width = .9, jitter.width = 0.2), 
      alpha = 0.5, aes(color = hi_lo)) + 
    expand_limits(y = 0) + theme_bw() + theme(legend.position = 'bottom') + 
    labs(title = 'Alpha diversity of communities\nby their endpoint colonization level',
      x = 'C difficile Challenge', y = 'Time Point of Diversity Metric', color = '',
      subtitle = paste0('Low = Cipro and Clinda\nHigh = Amp, Cef, Metro, Strep, Vanco'
        ))+ 
    scale_color_manual(breaks = c('FALSE', 'TRUE', 'High', 'Low'),
      labels = c('No CDI', 'CDI', 'High CFU', 'Low CFU'),
      values = c('gray', 'red', 'blue', 'black'))
...
abx_v_final_alpha %>% 
  ...
  ggplot(aes(x = time_point, y = value, color = hi_lo, group = interaction(hi_lo, time_point))) + 
    ...
    geom_point() + 
    expand_limits(y = 0) + 
    theme_bw() + labs(title = 'Alpha diversity of communities infected with C. difficile\nby their endpoint colonization level',
...
sig_relabun_by_day_df %>% 
  left_join(
    get_taxa_labels(tax_file, 'genus', unique(.$otu)) %>% 
      select(otu, tax_otu_label)) %>% 
  ggplot(aes(x = tax_otu_label, y = rel_abund, color = colonization)) + 
    stat_summary(fun.data=median_hilow, position=position_dodge(0.2), fun.args=list(conf.int=0.5)) +
...
del_beta_preabx <- beta_div_df %>% 
  mutate(preabx_sample = group) %>% 
  separate(group, c('preabx_mouse', 'preabx_day'), sep = 'D') %>% 
  gather(group, thetayc, -preabx_sample, -preabx_mouse, -preabx_day) %>% 
  separate(group, c('sample_mouse', 'sample_day'), sep = 'D', remove = F) %>% 
  mutate(thetayc = as.numeric(thetayc), preabx_day = as.numeric(preabx_day), sample_day = as.numeric(sample_day)) %>% 
  filter(preabx_day < 0, preabx_mouse == sample_mouse) %>%  # need to filter intial day since mouse 106-1 is missing samples prior to day 2
  group_by(preabx_mouse) %>% 
  filter(preabx_day == min(preabx_day)) %>% 
  ungroup %>% 
  filter(sample_day > 0) %>% 
  left_join(select(meta_file, -mouse, -day), by = 'group')

#######select preAbx by column########del_beta_preabx %>% 
#######select preAbx by column########  filter(cdiff == TRUE) %>% 
#######select preAbx by column########  ggplot(aes(x = thetayc, y = CFU)) + 
#######select preAbx by column########    scale_y_log10() + 
#######select preAbx by column########    geom_point(aes(color = abx)) + 
#######select preAbx by column########    geom_smooth(method = 'lm') +
#######select preAbx by column########    theme_bw() + 
#######select preAbx by column########    labs(x = 'Theta YC', y = 'C. difficile CFU (log10)', title = 'Beta diversity relative to preAbx VS C. difficile colonization\n')

del_beta_preabx %>% 
  filter(CFU == 0) %>% 
  ggplot(aes(thetayc)) + 
    geom_density(aes(color = abx))
...
corr_otu <- cor_df %>%
  ...
  filter(pvalue_bon <= 0.05, median_abundance > 0) %>% 
  arrange(rho)
...
cor_df  %>% 
  select(CFU, abx, one_of(corr_otu$otu)) %>% 
  gather(otu, rel_abund, contains('Otu00')) %>% 
  left_join(
    get_taxa_labels(tax_file, 'genus', unique(.$otu)) %>% 
      select(otu, tax_otu_label)) %>% 
  ggplot(aes(y = CFU, x = rel_abund)) + 
      geom_point(aes(color = abx)) + 
      scale_y_log10() + 
      geom_smooth(method = 'lm') +
      theme_bw() + 
      facet_wrap(.~tax_otu_label, ncol = 3, scales = 'free_x') + 
