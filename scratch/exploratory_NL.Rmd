...
del_beta_preabx <- beta_div_df %>% 
  left_join(select(meta_file, group, preAbx), by = 'group') %>% 
  filter(preAbx == TRUE) %>% 
  gather(sample_group, thetayc, -preAbx, -group) %>% 
  separate(group, c('preAbx_mouse', 'preAbx_day'), sep = 'D') %>% 
  separate(sample_group, c('sample_mouse', 'sample_day'), sep = 'D', remove = F) %>% 
  filter(preAbx_mouse == sample_mouse) %>% 
  mutate(thetayc = as.numeric(thetayc), preabx_day = as.numeric(preAbx_day), sample_day = as.numeric(sample_day)) %>% 
  filter(preAbx_day != sample_day) %>% 
  left_join(select(meta_file, -mouse, -day), by = c('sample_group' = 'group'))

del_beta_preabx %>% 
  filter(cdiff == TRUE) %>% 
  ggplot(aes(x = thetayc, y = CFU)) + 
    scale_y_log10() + 
    geom_point(aes(color = abx)) + 
    geom_smooth(method = 'lm') +
    theme_bw() + 
    labs(x = 'Theta YC', y = 'C. difficile CFU (log10)', title = 'Beta diversity relative to preAbx VS post C. difficile challenge')

del_beta_preabx %>% 
  filter(cdiff == TRUE, CFU == 0) %>% 
  ggplot(aes(thetayc)) + 
    geom_density(aes(color = abx)) + 
    theme_bw() + 
    labs(x = 'Theta YC', y = 'Sample density', title = 'Distribution of sample beta diversity with no CFU detected\n(comparing post C. difficile challenge to preAbx samples')
