...
initial_v_final <- hi_lo_no_delay_df  %>% 
  ...
  filter(!(time_point == 'initial' & preAbx == F)) %>% # need to remove samples that dont have a sample prior to abx treatment
...
```{r alpha of initial vs end, fig.width=4, fig.height=3}

initial_v_final %>% 
  left_join(alpha_df) %>% 
  filter(sobs > 15) %>% 
  gather(diversity, value, sobs, shannon, invsimpson) %>% 
  ggplot(aes(x = time_point, y = value, color = log_CFU)) + 
...
```{r end vs initial, fig.width = 5, fig.height = 5}
...
initial_v_final  %>% 
  ...
  ggplot(aes(x = axis1, y = axis2, color = time_point)) +
    geom_point(aes(size = log_CFU, shape = cdiff), alpha = 0.5) + 
    theme_bw() +
    labs(title = 'Comparison of difference between initial and final communities \nand their level of C difficile colonization')
...
CFU_intial_end <- initial_v_final %>% 
  group_by(cage, mouse) %>% 
  mutate(CFU = max(CFU), log_CFU = max(log_CFU)) %>% ungroup() %>% 
  select(-day, -preAbx) %>% 
  spread(time_point, group) %>% 
  filter(cdiff == T) %>% 
  filter(!is.na(end), !is.na(initial)) %>% 
  select(log_CFU, end, initial) %>% 
  mutate(thetayc = unlist(as.numeric(apply(., 1, function(i){
    beta_div_df[beta_div_df$group == i['initial'], i['end']]
  }))))

CFU_intial_end %>% 
  ggplot(aes(x = thetayc, y = log_CFU)) +
    geom_point() +
    theme_bw() + 
    labs(title = 'Relationship between initial vs final microbiota and final CFU',
