...
meta_cleaned  %>% 
  group_by(cage, mouse) %>% 
  summarize(initial = min(day), end = max(day)) %>% 
  mutate(initial_group = paste0(str_pad(cage, 3, pad= '0'), '-', mouse, "D", initial)) %>% 
  mutate(end_group = paste0(str_pad(cage, 3, pad= '0'), '-', mouse, "D", end))
