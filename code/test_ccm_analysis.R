...
#	current_otu2 <- 'EL'
	...
	set.seed(seed)
	print(paste0('Beginning ', current_otu1, ' and ', current_otu2, ' in from the test set'))

	ccm_run_results <- lapply(1:500, function(i){
		ccm_df <- setup_df_for_mccm(input_df, exp_list, n_mice)
		...
		maxE <- 5 #length(unique(abx_df$day)) - 2 # one less for separating NAs and one less sample
	...
	dynamics_plot <- test_set %>%
		select(unique_id, day, one_of(current_otu1, current_otu2)) %>% 
		gather(bacteria, counts, one_of(current_otu1, current_otu2)) %>% 
			ggplot(aes(x = day, y = counts, color = unique_id)) + 
				geom_line(alpha = 0.4) + 
				geom_point() + 
...
#		full_join(ccm_data)
	...
	if(min(ccm_data$ccm_p_value) < 0.05/10^5){ # ~100,000 comparisons across all treatments
		ggsave(filename = paste0(save_dir, treatment_subset, '/sig_ccm_', current_otu1, 
				'_', current_otu2, '_first_diff.jpg'),
