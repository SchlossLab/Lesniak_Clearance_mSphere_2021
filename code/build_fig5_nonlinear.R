...
cor_files <- list.files(path= paste0('data/process/', model_dir), 
  pattern='combined_all_imp_features_cor_.*', full.names = TRUE)
...
for(file_name in cor_files){
  ...
  get_interp_info(importance_data, model_name) %>%
    as.data.frame() %>%
    write_tsv(., paste0("data/process/", model_dir, "/", model_name, "_cor_importance.tsv"))
}
...
for(file_name in non_cor_files){
  ...
  get_interp_info(importance_data, model_name) %>%
    as.data.frame() %>%
    write_tsv(., paste0("data/process/", model_dir, "/", model_name, "_non_cor_importance.tsv"))
}
# ----------------------------------------------------------------------------->
...
best_files <- list.files(path= paste0('data/process/', model_dir), 
  pattern='combined_best.*', full.names = TRUE)

logit <- read_files(best_files[1])
#l2svm <- read_files(best_files[3])
#l1svm <- read_files(best_files[2])
#rbf <- read_files(best_files[6])
rf <- read_files(best_files[2])
...
top_20_otus <-  function(name){
  ...
  data_first_20 <- read.delim(paste0("data/process/", model_dir, "/", name, "_non_cor_importance.tsv"),
                                header=T,
                                sep='\t') %>%
    arrange(imp) %>%
    head(20)
...
intersect_four <- Reduce(intersect, list(NULL, rf_top$names))#, xgboost_top$names, rbf_top$names))
#intersect_three <- setdiff(Reduce(intersect, list(dt_top$names, rf_top$names, xgboost_top$names)), intersect_four)


#intersect_rf_dt <- setdiff(intersect(dt_top$names,rf_top$names), c(intersect_four))
...
base_nonlin_plot <-  function(data, name){
  ...
  data_first_20 <- read.delim(paste0("data/process/", model_dir, "/", name, "_non_cor_importance.tsv"), header=T, sep='\t') %>%
    arrange(imp) %>%
    head(20) 
  ...
  data_full <- read_files(paste0("data/process/", model_dir, "/combined_all_imp_features_non_cor_results_", name, ".csv")) %>%
    ...
    mutate(common_otus = case_when(names %in% intersect_four ~ "all models",
                                   ...
                                   TRUE ~ "no overlap")) %>%
    #inner_join(select(tax_df, OTU, tax_otu_label), by = c('names' = 'OTU')) %>% 
    #select(-names) %>% 
    #rename(names = tax_otu_label) %>% 
    group_by(names)
  ...
  if(name == "L1_Linear_SVM" || name =="L2_Linear_SVM" || name =="L2_Logistic_Regression"){
    ...
    corr_file <- paste0("data/process/", model_dir, "/", name, "_cor_importance.tsv")
    if(exists(corr_file)){
      data_cor_results <- read.delim(corr_file, header=T, sep='\t') %>%
...
# Extract legend
legend <- get_legend(rf_plot + guides(fill=guide_legend(ncol=2)) + theme(legend.position="bottom", 
...
#perm_tree_based <- plot_grid(dt_plot, rf_plot, labels = c("A", "B"), cols=1, scale = 0.97, align = "v")

non_linear_plot <- ggdraw(add_sub(rf_plot, "AUROC with the OTU permuted randomly", size=10, vpadding=grid::unit(0,"lines"), y=5, x=0.65, vjust=4.75))

#plot <- plot_grid(last_plot(), legend, ncol = 1, rel_heights=c(.92, .1))

ggsave(paste0("results/figures/Figure_5_nonlinear_", model_dir, ".jpg"), 
	plot = non_linear_plot, width = 6, height = 9)
