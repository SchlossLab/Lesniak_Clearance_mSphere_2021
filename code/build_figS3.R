...
shared_level_df <- sum_otu_by_taxa(tax_df, shared_df, taxa_level = 'Family', silent = T)
...
meta_abund_df <- meta_df %>%
	...
	inner_join(shared_level_df, by = c('group' = 'Group')) %>% 
	select(group, day, abx, mouse_id, taxa, clearance, time_point, abundance) %>% 
...
###############################################################################
#   What genus/family are associated with clearance?
####
...
pval_diff_colon_clear_df <- meta_abund_df %>% 
	...
	group_by(abx, clearance, time_point, taxa) %>% # compare each OTU abundance within each antibiotic in each time point 
	mutate(median_abundance = median(abundance)) %>%
	group_by(abx, time_point, taxa) %>% 
...
#	left_join(select(tax_df, OTU, taxa), by = 'OTU') # add otu labels
...
pval_diff_colon_clear_df <- pval_diff_colon_clear_df %>% 
	full_join(colon_clear_median_df, by = c('abx', 'taxa', 'time_point')) %>% 
...
diff_abund_clear_colon_plot <- pval_diff_colon_clear_df %>% 
	ggplot(aes(x = -order, color = time_point)) + 
		...
		scale_x_continuous(breaks = -colon_clear_otu_label$order, 
			labels = colon_clear_otu_label$taxa, expand = c(0,0)) + 
		# limit of detection
...
pval_diff_cleared_df <- meta_abund_df %>% 
	...
	group_by(abx, taxa, clearance) %>% # compare OTUs across timepoints within each antibiotic
...
lod_label_df <- pval_diff_cleared_df %>% 
	...
	select(abx, taxa, clearance, order) %>% 
	unique %>% 
...
pval_diff_cleared_df <- pval_diff_cleared_df %>% 
	select(abx, clearance, taxa) %>% 
	distinct %>% 
	group_by(abx) %>% 
	count(taxa) %>% 
	mutate(both = n > 1) %>% 
	full_join(pval_diff_cleared_df, by = c('abx', 'taxa')) %>% 
	ungroup
...
plot_temporal_diff_by_clearance <- function(end_status){
	plot_df <- pval_diff_cleared_df %>% 
		full_join(cleared_median_df, by = c('abx', 'clearance', 'comparison', 'taxa')) %>% 
		filter(clearance == end_status) %>% 
		mutate(taxa = gsub('_unclassified', '', taxa))
	# create label df to eliminate over plotting of labels and bold OTUs in both comparisons
	otu_label <- plot_df %>% 
		select(abx, comparison, order, taxa, both) %>% 
		mutate(taxa = paste0('*', taxa, '*'),
			taxa = ifelse(both == T, paste0('**', taxa, '**'), 
				taxa)) %>% 
		unique
	...
	output_plot <- plot_df %>% 
		ggplot(aes(x = -order, color = time_point)) + 
			...
			scale_x_continuous(breaks = -otu_label$order, 
				labels = otu_label$taxa, expand = c(0,0)) + 
			...
			facet_grid(abx~comparison, scales = 'free_y', space = 'free',
				labeller = labeller(comparison = c(Initial_TOI = "Initial vs Time of infection", 
					TOI_End = "Time of infection vs End of experiment"))) + 
			scale_y_log10(limits = c(0.04,100),
		   		breaks = c(0.01, 0.1, 1, 10, 100),
		   		labels = c('10^-2', '10^-1', '10^0', '10^1', '10^2')) + 
			coord_flip() + theme_bw() +  
...
ggsave(paste0('results/figures/figure_S3.jpg'), 
	plot_grid(
		plot_grid(NULL, plot_grid(diff_abund_clear_colon_plot, labels = c('A')), NULL, rel_widths = c(1, 5, 1), nrow = 1), 
		plot_grid(diff_abund_cleared_plot, diff_abund_colon_plot, labels = c('B', NULL), nrow = 1),
