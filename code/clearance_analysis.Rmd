...
se_cdiff_df <- meta_file %>% 
	...
	filter(cdiff == T, clearance != 'colonized', day > 0) %>% 
	inner_join(rel_abund, by = 'group') %>% 
...
n.c <- symBeta(getOptBeta(se.mb.cdiff))
colnames(n.c) <- rownames(n.c) <- colnames(se_cdiff_df)
...
cdiff.coord <- layout.fruchterman.reingold(stool.ig)
plot(ig.mb, layout=cdiff.coord, vertex.size=vsize, vertex.label=NA, main="mb")
plot(ig.gl, layout=cdiff.coord, vertex.size=vsize, vertex.label=NA, main="gl")
plot(stool.ig, layout=cdiff.coord, vertex.size=vsize, vertex.label=NA, main="mb")
