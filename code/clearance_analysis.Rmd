...
test_otus <- function(antibiotic, time_point){
	...
	## select otus present in at least 10% of samples
...
differential_abundance_plot <- function(df, treatment_group, time_point){
	lod_df <- data.frame(x = 1.5, y = 1/20)
	temp_df <- df %>% 
	...
	temp_df %>% 
		...
		right_join(., group_by(., OTU, clearance) %>% 
				summarise(median_abundance = (median(abundance)/20) + 0.04),
			by = c('OTU', 'clearance')) %>% 
		...
		ggplot(aes(x = reorder(tax_pval, -order), color = clearance)) + 
			...
			geom_point(aes(y = (abundance/20) + 0.04), 
			...
			theme_bw() +  
			labs(x = NULL, y = 'Relative Abundance (%)', title = paste0('Differentially abundant OTUs for ', 
...
differential_abundance_plot(test_output_df, 'cef', 0)
differential_abundance_plot(test_output_df, 'metro', 0)
differential_abundance_plot(test_output_df, 'strep', 0)
differential_abundance_plot(test_output_df, paste(treatment_list[[4]], collapse = '_'), 0)
differential_abundance_plot(test_output_df, paste(treatment_list[[5]], collapse = '_'), 0)
differential_abundance_plot(test_output_df, 'cef', 'last_sample')
differential_abundance_plot(test_output_df, 'metro', 'last_sample')
differential_abundance_plot(test_output_df, 'strep', 'last_sample')
differential_abundance_plot(test_output_df, paste(treatment_list[[4]], collapse = '_'), 'last_sample')
differential_abundance_plot(test_output_df, paste(treatment_list[[5]], collapse = '_'), 'last_sample')
