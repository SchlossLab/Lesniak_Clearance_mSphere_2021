...
test_otus <- function(antibiotic, time_point){
	...
	if(nrow(sig_otus) < 1){
			print(paste('No significant OTUs for comparison -', antibiotic, 
...
treatment_group <- treatment_list[[3]]
time_point <- 0
differential_abundance_plot <- function(df, treatment_group, time_point){
	df %>% 
		filter(antibiotic == treatment_group, time_point == time_point) %>% 
		...
		right_join(., group_by(., OTU) %>% 
			...
			by = c('OTU')) %>% 
		ggplot(aes(x = reorder(tax_otu_label, -order), y = ((1 + abundance)/20), 
				color = clearance)) + 
			geom_sina(alpha = 0.3) +
			theme_bw() + 
			coord_flip() + 
			scale_y_log10() + 
			labs(x = NULL, y = 'Abundance', title = paste0('Differentially abundant OTUs for ', 
				paste(treatment_group, collapse = ' '), ', for samples on ', 
				unique(df$time_point), ' day')) + 
...
test_output_df %>% 
		...
		right_join(., group_by(., OTU, clearance) %>% 
				summarise(median_abundance = median(abundance)/20),
			by = c('OTU', 'clearance')) %>% 
		select(-group, -abundance) %>% distinct %>% 
		spread(clearance, median_abundance) %>% 
		ggplot(aes(y = reorder(tax_otu_label, -order), x = cleared, xend =colonized)) + 
			geom_dumbbell(colour_x="#edae52", 
                      colour_xend = "#9fb059",
                      size_x = 3, size_xend = 3) + 
			scale_x_log10() + 
			theme_bw() 
			labs(y = NULL, x = 'Abundance', title = paste0('Differentially abundant OTUs for ', 
				paste(treatment_group, collapse = ' '), ', for samples on ', 
				unique(df$time_point), ' day')) + 
			theme(legend.title = 'none')
...
differential_abundance_plot(d0_test_output_df, 'cef')
differential_abundance_plot(d0_test_output_df, 'metro')
differential_abundance_plot(d0_test_output_df, 'strep')
differential_abundance_plot(d0_test_output_df, paste(treatment_list[[4]], collapse = '_'))
differential_abundance_plot(d0_test_output_df, paste(treatment_list[[5]], collapse = '_'))
differential_abundance_plot(end_test_output_df, 'cef')
differential_abundance_plot(end_test_output_df, 'metro')
differential_abundance_plot(end_test_output_df, 'strep')
differential_abundance_plot(end_test_output_df, paste(treatment_list[[4]], collapse = '_'))
differential_abundance_plot(end_test_output_df, paste(treatment_list[[5]], collapse = '_'))
