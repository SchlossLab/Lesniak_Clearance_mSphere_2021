...
test_otus_d0 <- function(antibiotic){
	d0_shared_clearance <- meta_file %>% 
		left_join(clearance, by = c('mouse_id')) %>% 
		filter(day == 0, cdiff == T, abx %in% antibiotic, clearance != 'clearing') %>% 
	...
	if(nrow(filter(sig_otus, pvalue < 0.05/length(d0_otus))) < 1){
			sig_otus <- sig_otus %>% 
				top_n(-10, pvalue)
		} else {	
			sig_otus <- sig_otus %>% 
	...
	d0_shared_clearance <- d0_shared_clearance %>% 
	...
	return(data.frame(d0_shared_clearance, antibiotic = paste(antibiotic, collapse = '_'), stringsAsFactors = F)) 
}

test_output_df <- map_dfr(list('cef', 'metro', 'strep', c('cef', 'metro', 'strep'), 
		c("clinda", "vanc", "cipro", "amp", "cef", "metro", "strep")), ~ test_otus_d0(.x))

test_output_df %>% 
	filter(antibiotic == 'cef') %>% 
	mutate(tax_pval = paste0(tax_otu_label, '\n(p=', 
		formatC(pvalue, format = "e", digits = 2), ')')) %>% 
	ggplot(aes(x = clearance, y = abundance, color = clearance)) + 
		geom_jitter() + 
		facet_wrap(. ~ tax_pval, scales = 'free_y') +
		labs(x = NULL, y = 'Abundance', title = paste('Differentially abundant OTUs in', paste(antibiotic, collapse = ' '))) + 
		theme(legend.position = 'none')

test_output_df %>% 
	select(antibiotic, tax_otu_label, pvalue) %>% 
	unique %>% 
	pull(tax_otu_label) %>% 
	table
