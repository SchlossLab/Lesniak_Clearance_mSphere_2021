...
#  pval_a_cause_b pval_b_cause_a cdiff_cause_otu otu_cause_cdiff       otu
#            0.68           0.03    0.0875640809       0.6976916 Otu000022
#
# Otu000022	491862	Bacteria(100);Bacteroidetes(100);Bacteroidia(100);Bacteroidales(100);Rikenellaceae(100);Alistipes(100);

Accm<-test_df$CFU
Bccm<-test_df$Otu000006
#Calculate optimal E
maxE<-9 #Maximum E to test
#Matrix for storing output
Emat<-matrix(nrow=maxE-1, ncol=2); colnames(Emat)<-c("A", "B")
...
meta_file %>% 
	...
	left_join(select(shared_file, -label, -numOtus),
		by = c('group' = "Group")) %>%
	select(cage, mouse, day, CFU, Otu000006) %>% 
	gather(bacteria, counts, CFU, Otu000006) %>% 
