...
# based on previously published papers, most leave C difficile in 
# I had been initially removing from when I was trying to predict C difficile 
# but that does not seem to be necessary based on what I am doing and others 
# have published
#
...
#
# remove C. difficile sequences
# look at the distribution of C. difficile sequences
#shared_cdiff_df <- select(shared_df, one_of(cdiff_otus))  %>% 
#  mutate(group = rownames(shared_df)) %>% 
#  gather(OTU, counts, contains('Otu00')) %>% 
#  left_join(select(meta_df, day, abx, group, CFU))
#shared_cdiff_df <- bind_rows(shared_cdiff_df, 
#  summarise(group_by(shared_cdiff_df, day, abx, group, CFU), counts = sum(counts)) %>% 
#    mutate(OTU = "Summed"))
# prior to challenge, 3 or more cdifficile seq are detected in 84 samples
# maximum count is 65 and median is 6
# post infection, 586 samples have 3 or more sequences
# maximum count is 1083 and median is 12
# for most of the samples with Cdifficile are in the first 3 days post infection
# of note a majority of the samples with higher counts are with cef treatment
# cages 104, 105, and 600 had the most high counts of cdifficile sequences

# all samples have otus counts totalling 2000, 
# but the C difficile counts and otus present below the cutoff with be removed
# Jenior etal 2018 removed Cdiff OTU before calculating relative abundance using the genus Clostridium_XI)
#

shared_df <- select(shared_df, -label, -numOtus)

sample_summed_counts <- apply(shared_df, 1, sum)
rel_abund <- 100 * shared_df/sample_summed_counts
