...
compare_community_colonization <- function(selected_treatment, taxonomic_level, n_taxa){
	...
	plot_header <- ggdraw() +
	 draw_label(paste0('Daily Community overview ', treatment, ' looking at the ', 
	    taxonomic_level), x = 0.05, hjust = 0)

    ggsave(paste0('../results/figures/daily_plots/colonization_dynamics', treatment, '_', taxonomic_level, '.jpg'),
      plot_grid(plot_header, plot_grid(community_plot, cdiff_plot, ncol = 1), 
        ncol = 1, rel_heights = c(0.1, 2)),
      width = 20, height = 10)
  }

compare_community_colonization(selected_treatment = 'clinda_10_FALSE_TRUE', 
	taxonomic_level = 'tax_otu_label', n_taxa = 6)

for(trt in unique(meta_df$treatment)){
	for(tax in 1:6){
