...
permutation_importance <- function(model, full){
  ...
  print('Calculating test ROC')
  base_roc <- roc(ifelse(full$dx == "Colonized", 1, 0), rpartProbs[[1]])
  ...
  non_corr_imp <- do.call('rbind', lapply(non_correlated_otus, function(i){
    ...
    new_auc <- roc(ifelse(full_permuted$dx == "Colonized", 1, 0), rpartProbs_permuted[[1]])$auc
  ...
  groups <- lapply(1:n_split, function(i){
  ...
  groups_list <- map(groups[1:n_split], "column")
  groups_list_sorted <- map(groups_list[1:n_split], sort)
  ...
  # This goes all the way to n_split in the list, because there are n_split groups of correlated OTU groups.
  # ----------------------------------------------------------------------------- #
  ...
  corr_imp <- do.call('rbind', lapply(groups_list_sorted, function(i){
    ...
    full_permuted_corr[,unlist(i)] <- sample(full[,unlist(i)])
    ...
    # Calculate the new auc
    new_auc <- roc(ifelse(full_permuted_corr$dx == "Colonized", 1, 0), rpartProbs_permuted_corr[[1]])$auc
  ...
  # We use seperate function to break up the grouped list of OTUs
  ...
  max_corr_otus <- max(sapply(corr_imp[,2], function(i){length(unlist(i))}))
  x <- as.character(seq(1, max_corr_otus, 1))
